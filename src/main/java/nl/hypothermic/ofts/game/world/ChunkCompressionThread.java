...
public final class ChunkCompressionThread implements Runnable {
	...
	private final HashMap<AcceptedConnection, Integer> queueSizePerPlayer = new HashMap<AcceptedConnection, Integer>();
	...
	private void handleMapChunk(QueuedPacket queuedPacket) {
		Packet51 packet = (Packet51) queuedPacket.packet;

		// If 'packet.g' is set then this packet has already been compressed. // OFTS: 'packet.chunkData'
		if (packet.chunkData != null) {
			return;
		}

		int dataSize = packet.chunkData.length;
	...
	private void sendToNetworkQueue(QueuedPacket queuedPacket) {
		queuedPacket.player.queuePacket(queuedPacket.packet);
	}

	public static void sendPacket(AcceptedConnection player, Packet... packets) {
		for (Packet packet : packets) {
			if (packet instanceof Packet51) {
	...
	private void addToPlayerQueueSize(AcceptedConnection player, int amount) {
	...
	public static int getPlayerQueueSize(AcceptedConnection player) {
	...
	private static class QueuedPacket {
		final AcceptedConnection player;
		final Packet packet;
		final boolean compress;

		QueuedPacket(AcceptedConnection player, Packet packet, boolean compress) {
			this.player = player;
			this.packet = packet;
