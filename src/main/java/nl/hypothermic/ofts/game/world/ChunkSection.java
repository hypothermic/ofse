...
public class ChunkSection {
	...
	// Renamed from "ExtendedBlockStorage" to "ChunkSection" for easier typing.
				...
				for (int var3 = 0; var3 < 16; ++var3) {
					...
					if (var4 > 0) {
						if (Block.byId[var4] == null) {
							...
							if (Block.byId[var4].needsRandomTick) {
								++this.tickRefCount;
							}
						}
					}
	...
	public void setExtBlockID(int par1, int par2, int par3, int par4) {
		...
		if (var5 == 0 && par4 != 0) {
			...
			if (Block.byId[par4] != null && Block.byId[par4].needsRandomTick) {
				++this.tickRefCount;
			}
		} else if (var5 != 0 && par4 == 0) {
			--this.blockRefCount;

			if (Block.byId[var5] != null && Block.byId[var5].needsRandomTick) {
				--this.tickRefCount;
			}
		} else if (Block.byId[var5] != null && Block.byId[var5].needsRandomTick && (Block.byId[par4] == null || !Block.byId[par4].needsRandomTick)) {
			--this.tickRefCount;
		} else if ((Block.byId[var5] == null || !Block.byId[var5].needsRandomTick) && Block.byId[par4] != null && Block.byId[par4].needsRandomTick) {
			++this.tickRefCount;
		}
