...
public class OldChunkLoader {
    ...
    public static OldChunk a(NBTTagCompound nbttagcompound) {
        ...
        oldchunk.h = nbttagcompound.getTagList("Entities");
        oldchunk.i = nbttagcompound.getTagList("TileEntities");
        oldchunk.j = nbttagcompound.getTagList("TileTicks");

        try {
            oldchunk.a = nbttagcompound.getLong("LastUpdate");
        } catch (ClassCastException classcastexception) {
            oldchunk.a = (long) nbttagcompound.getInteger("LastUpdate");
        }
    ...
    public static void a(OldChunk oldchunk, NBTTagCompound nbttagcompound, WorldChunkManager worldchunkmanager) {
        nbttagcompound.setInteger("xPos", oldchunk.k);
        nbttagcompound.setInteger("zPos", oldchunk.l);
        ...
        for (int k = 0; k < 8; ++k) {
            ...
            if (!flag) {
                ...
                nbttaglist.appendTag(nbttagcompound1); // ofts: original: "nbttaglist.add(...)"
            }
        }
        ...
        nbttagcompound.setTag("Entities", oldchunk.h);
        nbttagcompound.setTag("TileEntities", oldchunk.i);
        if (oldchunk.j != null) {
            nbttagcompound.setTag("TileTicks", oldchunk.j);
        }
    }
}
